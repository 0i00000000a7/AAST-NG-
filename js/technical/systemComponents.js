var systemComponents = {
  ...
  'tree-node': {
    ...
    template: `
		<button v-if="nodeShown(layer)"
			v-bind:id="layer"
			v-on:click="function() {
				if ((shiftDown||shitDown) && options.forceTooltips && layer!=='t') player[layer].forceTooltip = !player[layer].forceTooltip
  ...
  'overlay-head': {
    template: `			
		...
		<span v-if="layers.te.devSpeedCal && layers.te.devSpeedCal() != 1" class="overlayThing">
			<br>Dev Speed: {{format(layers.te.devSpeedCal())}}x<br>
		</span>
  ...
  'info-tab': {
    template: `
        ...
        <span v-if="modInfo.author">
            <br>
            <span v-html="modInfo.author"></span>
        </span>
        <br>
